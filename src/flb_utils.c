...
static int flb_utils_write_str_escaped(char *buf, int *off, size_t size, const char *str, size_t str_len)
{
    ...
    for (i = 0;;) {
         ...
         * Only take steps that fit entirely inside the string: after the
         * byte-by-byte loop below, 'i' can resume unaligned, and a bound of
         * 'i < vlen' would let the final iteration read and bulk-copy up to
         * inst_len - 1 bytes past str_len (out-of-bounds read plus garbage
         * from adjacent memory appended to the output).
         */
