                                       ...
                                       struct flb_aws_credentials *creds)
{
    ...
    /*
     * Build the presigned URL and base64url-encode it. The User-Agent
     * parameter is part of the signed URL, matching the Go implementation.
     * The encoding is RawURLEncoding: '+' becomes '-', '/' becomes '_' and
    ...
    tmp = flb_sds_printf(&presigned_url,
                         "https://%s/?%s&User-Agent=fluent-bit-msk-iam",
                         host, query);
    if (!tmp) {
        goto error;
    }
    ...
    encoded_len = ((url_len + 2) / 3) * 4 + 1; /* Base64 size + null terminator */
    ...
    encode_result = flb_base64_encode((unsigned char *) payload, encoded_len,
                                      &actual_encoded_len,
